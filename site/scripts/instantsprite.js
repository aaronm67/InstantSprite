...
sprite.mergefiles = function() {
	...
	if (isPacked) {
		...
		// growing packer requires elements to be pushed roughly in order of size to work properly
		canvases = _.sortBy(canvases, function(c) { 
			return -c.w * c.h;
		});

		var packer = new GrowingPacker(canvases);

		var i = 0;
		sprite.eachcanvas(function(c) {
			var obj = _.find(packer.blocks, function(o) { return o.idx === i });
			c.storeX = obj.fit.x;
			c.storeY = obj.fit.y;
			i++;
		})
